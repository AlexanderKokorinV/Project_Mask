...
class TestGetTransactionsFromJson(unittest.TestCase):
    ...
    def test_get_transactions_from_json(self, mock_file_open: Mock) -> None:
        """
        Тест успешного чтения корректного JSON-файла
        """
        mock_file_open.return_value.read.return_value = (
            self.MOCK_JSON_STR
        )  # Настраиваем Mock-объект так, чтобы он "читал" наши данные
        ...
        mock_file_open.assert_called_once_with(
            "fake/path/to/operations.json", encoding="utf-8"
        )  # Проверка, что функция пыталась открыть файл с нужными параметрами

    @patch("builtins.open", new_callable=mock_open)
    def test_file_not_found(self, mock_file_open: Mock) -> None:
        """
        Тест обработки ситуации, когда файл не найден (FileNotFoundError).
        """
        mock_file_open.side_effect = (
            FileNotFoundError  # Заставляем mock_open выбросить FileNotFoundError при попытке открыть файл
        )
    ...
    @patch("builtins.open", new_callable=mock_open)
    def test_json_decode_error(self, mock_file_open: Mock) -> None:
        """
        Тест обработки ситуации, когда JSON файл поврежден или содержит не список (json.JSONDecodeError).
        """
        mock_file_open.return_value.read.return_value = (
            "Это невалидный JSON"  # Имитируем файл, содержащий невалидный JSON
        )
        ...
        mock_file_open.assert_called_with("invalid/file.json", encoding="utf-8")

    @patch("builtins.open", new_callable=mock_open)
    def test_empty_file_handling(self, mock_file_open: Mock) -> None:
        """
        Тест обработки пустого файла, который вызывает ValueError.
        """
