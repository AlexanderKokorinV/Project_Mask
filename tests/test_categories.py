...
def test_count_categories(transactions: List[Dict], descriptions: List) -> None:
    ...
    assert count_categories(transactions, descriptions) == {"Перевод организации": 2, "Перевод со счета на счет": 2, "Перевод с карты на карту": 1}


@pytest.mark.parametrize("transactions, descriptions, expected", [
    (
